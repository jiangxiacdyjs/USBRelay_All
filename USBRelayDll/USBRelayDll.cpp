...
// 继电器初始化
JNIEXPORT jint JNICALL Java_indi_whc_webandcv_Java2Cpp_SystemInit(JNIEnv *, jclass){

	return usb_relay_init();  // 继电器初始化
	cout << "SystemInit" << endl;

}

// 查找插入到电脑中的所有USB免驱继电器模块
JNIEXPORT jint JNICALL Java_indi_whc_webandcv_Java2Cpp_FindDevice(JNIEnv *, jclass) {
	...
	DeviceList = usb_relay_device_enumerate(); // 查找插入到电脑中的所有USB免驱继电器模块
...
// 选择指定的继电器设备 返回继电器设备句柄
JNIEXPORT jint JNICALL Java_indi_whc_webandcv_Java2Cpp_SelectDevice(JNIEnv *, jclass, jint device) {
...
// 继电器命令 打开指定继电器开发
JNIEXPORT jint JNICALL Java_indi_whc_webandcv_Java2Cpp_OpenRelay(JNIEnv *, jclass, jint hd, jint relay_index) {
	...
	if ((relay_index > 0) && (relay_index < 255)) {
		...
		if (result == 0)
			...
			cout << "|                   操作失败                      |" << endl;
	} else if (relay_index == 255) { // 打开继电器上所有的控制开关
...
// 继电器命令 关闭指定继电器开发
JNIEXPORT jint JNICALL Java_indi_whc_webandcv_Java2Cpp_CloseRelay(JNIEnv *, jclass, jint hd, jint relay_index) {
...
// 关闭继电器设备
JNIEXPORT void JNICALL Java_indi_whc_webandcv_Java2Cpp_CloseDevice(JNIEnv *, jclass, jint hd) {
	usb_relay_device_close(hd);
}

// 打开继电器设备 ，选择继电器设备<板子>
JNIEXPORT jint JNICALL Java_indi_whc_webandcv_Java2Cpp_SetDeviceSerialNum (JNIEnv *env, jclass, jstring SerialNum){
...
JNIEXPORT jint JNICALL Java_indi_whc_webandcv_Java2Cpp_SetRelayNum (JNIEnv *, jclass, jint relay) {
